...
const createProject = asyncHandler(async (req, res) => {
  ...
  if (!sanitizedInputs.name || !sanitizedInputs.description) {
...
const sendInvitation = asyncHandler(async (req, res) => {
  ...
  const existingInviteQuery = {
    ...
    invitedUser: user._id.toString(),
    project: req.params.id.toString(),
    status: "Pending",
  };

  // check for already sent invites.
  const existingInvite = await Invite.findOne(existingInviteQuery);

  if (existingInvite) {
    throw new APIError(409, "Invitation already sent to user");
  }

  const newInviteQuery = {
    invitedBy: req.user._id.toString(),
    invitedUser: user._id.toString(),
    project: req.params.id.toString(),
  };

  const newInvite = await Invite.create(newInviteQuery);
